...
def test_best_model_does_not_crash():
    call_llm(prompt="What are some adventure movies in this dataset?", retry=False)


def test_medium_model_does_not_crash():
    call_llm(prompt="Which movies were directed by James Cameron?", model_type=medium, retry=False)
